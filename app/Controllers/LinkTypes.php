...
class LinkTypes extends Controller {
  ...
  public function new() {
    ...
    if ($this->request->getMethod() === 'post') {
      ...
      if ($validation->withRequest($this->request)->run(null, null, 'publications')) {
        ...
        // Go back to index
        return redirect()->to(base_url() . "/linkTypes/index/".$page);
  ...
  public function delete() {
    ...
    if ($this->request->getMethod() === 'post') {
      ...
      // Go back to index
      return redirect()->to(base_url() . "/linkTypes/index");
  ...
  public function view() {
    ...
    if (logged_in() == false) {
      $linkTypeID = $uri->getSegment(4);
      $_SESSION['redirect_url'] = base_url() . '/linkTypes/view/1/' . $linkTypeID;
      return redirect()->to(base_url() . '/login');
    }
    ...
    // Set the session last page
    $session = session();
    $session->set('lastPage', 'LinkTypes::view');
    ...
    echo view('templates/header.php', $data);
    echo view('templates/menu.php', $data);
    echo view('linkTypes/view.php', $data);
    echo view('templates/footer.php', $data);
  }
