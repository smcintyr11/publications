...
class LocalAuthenticator extends AuthenticationBase implements AuthenticatorInterface
{
    ...
    public function attempt(array $credentials, bool $remember = null): bool
    {
        ...
        if (! $this->user->isActivated())
        {
            ...
            $this->error = lang('Auth.notActivated') .' '. anchor('/publications/resend-activate-account').'?'.$param, lang('Auth.activationResend'));
    ...
    public function check(): bool
    {
        if ($this->isLoggedIn())
        {
            ...
            {
                throw new RedirectException('/publications/reset-password') .'?token='.$this->user->reset_hash);
            }
